...
function selectResouce(url) { //#E
  ...
  var result;
  if (parts[0] === 'actions') {
    result = config.WoTModel.links.actions.resources[parts[1]].data;
  } else if (parts[0] === 'properties') {
    result = config.WoTModel.links.properties.resources[parts[1]].data;
  } else {
    result = config.WoTModel.things[parts[1]];
  }
